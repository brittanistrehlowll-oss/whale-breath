# install.ps1 — 鲸息 V1.0.3 安装（jingxi Skill 动作之一）
#
...
foreach ($targetHome in $targets) {
  ...
  if (Test-Path $patchPath) {
    ...
    if ($patch -match 'name:\s*dsh-jingxi') {
      ...
      $patch += "`n# dsh-jingxi V1.0.3 Pure Breath (installed by jingxi skill)`n- insert:`n    - id: jingxi`n      name: dsh-jingxi`n      inject: [webServer]`n"
